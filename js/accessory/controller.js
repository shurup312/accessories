/regular=[0-10]$\s+/;
var AccessoryDetailCtrl, AccessoryEditCtrl, AccessoryListCtrl, AccessoryNewCtrl;

AccessoryNewCtrl = function($scope, $routeParams, AccessoryAdd, AccessoryBreadcrumbs, $timeout) {
  AccessoryAdd.get({
...
AccessoryListCtrl = function($scope, $http, $routeParams, AccessoryList, AccessoryHierarchy, AccessoryBreadcrumbs, AccessoryDelete, $window, $timeout, $cookieStore) {
  ...
  $scope.goMassDelete = function() {
    ...
    for (_j = 0, _len1 = elements.length; _j < _len1; _j++) {
      element = elements[_j];
      _results.push(AccessoryDelete["delete"]({
        id: element
      }, function(ok) {
        okCount++;
...
AccessoryDetailCtrl = function($scope, $routeParams, $http, AccessoryDetail, AccessoryBreadcrumbs, $timeout) {
  AccessoryDetail.get({
    id: $routeParams.id
  }, function(ok) {
    return $scope.details = ok;
  }, function(error) {
    $scope.errorMessage = error.data;
    return $timeout(function() {
      return $scope.errorMessage = '';
