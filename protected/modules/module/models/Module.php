...
class Module extends CActiveRecord {
	...
	public function getList ($id) {
		try{
			...
			$arr = $db->createCommand()->select('m.can_delete, m.id id, m.name name, mmt.name type_name, mh.child_id_type child_type, m.sort')
					  ...
					  ->leftJoin('module_hierarchy mh', 'm.type_id=mh.parent_id_type')
	...
	public function getFieldsForType ($id) {
		try {
			$db       = $this->getDb();
			$query    = $db->createCommand()->select(
						   'tab.id tab_id, tab.name tab_name, mod_p.table param_table, mod_p.field param_field, mod_p.type field_type, param.name field_description, mod_p.exp, mod_p.exp_description')
	...
	public function getHierarchy ($id) {
		try {
			$db = $this->getDb();
			if ($id > 0) {
				return $db->createCommand()->select('mt.name name, mt.object_id id')->from('module m')->join(
						  'module_hierarchy mh',
				...
				)     ->where('mh.parent_id_type=0 and mh.module_id=1 and m.type_id=4')->queryAll();
			}
		} catch (Exception $e){
	...
	public function getData ($id, $tables, $fields){
		try {
			sizeof($tables)>1
				&& ($ext = $tables[0]!='module'?$tables[0]:$tables[1])
				|| $ext=false;
			$query = $this->getDb()->createCommand()->select(implode(', ', $fields))->from('module')->where(
						  'module.id=:id',
						  array (':id' => $id)
				);
			$ext && $query->join($ext,"$ext.id = module.object_id");
	...
	public function updateDate ($data){
		try {
			...
			$art = $db->createCommand()->select('*')->from('module')->where('id=:id',array(':id'=>$id))->queryRow();
	...
	public function deleteModule ($id){
		try {
			...
			$db->createCommand()->delete('module','id=:id',array(':id'=>$id));
	...
	public function getBreadcrumbs ($id){
		try {
			$db = $this->getDb();
			$query = $db->createCommand()->select('bc.id, bc.name')->from('module m')->join(
						'module bc',
